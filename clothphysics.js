...
var fabricLength = 230; // sets the size of the cloth
...
var clothInitialPosition = plane( 100, 230 );
...
function Particle( x, y, z, mass ) {
  ...
  this.position = clothInitialPosition( x, y, target ); // position
  this.previous = clothInitialPosition( x, y, target ); // previous
  this.original = clothInitialPosition( x, y, target ); // original
...
function simulate( time ) {
  ...
  if ( wind )
  {
    ...
    windForce.set(
      0.8 + Math.sin( time / 2000 ),
      Math.cos( time / 2500 ),
      0.3 + Math.sin( time / 3000 )
